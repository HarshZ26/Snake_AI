...
def gen_episode (env,alpha,gamma,epsilon): 
	...
	def get_state(ob,x,y,xf,yf,d):
		...
		def inc(ob,x,y,d):
			...
			if d==2:
				if y< y_max-1:
					if(np.array_equal(ob[10*(y+1)][10*x],np.array([1,0,0]))) or (np.array_equal(ob[10*(y+1)][10*x],np.array([255,10,0]))) or (np.array_equal(ob[10*(y+1)][10*x],np.array([255,20,0]))):
						dis = 0
				if dis==1 and y==(y_max-1):
					dis = 0
			else:
				if x< x_max-1:
					if(np.array_equal(ob[10*y][10*(x+1)],np.array([1,0,0]))) or (np.array_equal(ob[10*y][10*(x+1)],np.array([255,10,0]))) or (np.array_equal(ob[10*y][10*(x+1)],np.array([255,20,0]))):
						dis = 0
		...
		def dec(ob,x,y,d):
			...
			if d==0:
				...
				if x>0:
					if(np.array_equal(ob[10*y][10*(x-1)],np.array([1,0,0]))) or (np.array_equal(ob[10*y][10*(x-1)],np.array([255,10,0]))) or (np.array_equal(ob[10*y][10*(x-1)],np.array([255,10,0]))):
						dis = 0
				if dis==1 and x==0:
					dis=0
			return dis
	...
	while not done: 
		...
		x_f1,y_f1,x_f2,y_f2 = fru_lis[0],fru_lis[1],fru_lis[2],fru_lis[3]
		...
		dr_new1 = action1
		dr_new2 = action2
		obs,rew,done, info = env.step([action1,action2])
		if rew[0]==1 or rew[1]==1:
			fru_lis = get_fruit(obs)
			x_f1,y_f1,x_f2,y_f2 = fru_lis[0],fru_lis[1],fru_lis[2],fru_lis[3]
...
if __name__ =="__main__":
	...
	for i in range(0,ep):
		...
		print("episode "+str(i+1)+" ended.Reward = "+str(rew) + str(steps))
	...
	plt.plot(rew_list[0])
	plt.plot(rew_list[1])
	plt.legend(['Snake 1','Snake 2'])
	plt.savefig('qrew_mul.png')
	plt.clf()
	plt.plot(step_list[0])
	plt.plot(step_list[1])
	plt.legend(['Snake 1','Snake 2'])
	plt.savefig('q_steps_mul.png')
	plt.clf()
